...
from visual_testing_framework import VisualTestFramework, CircuitBehaviorValidator, ReferenceSignalGenerator
...
class TestACAnalysisBehavior:
    ...
    def test_ac_analysis_returns_complex_values(self, engine, rc_lowpass_circuit, behavior_validator):
        ...
        assert result.passed, f"AC analysis complex value validation failed: {result.issues}"
        assert result.score >= 0.8, f"AC analysis complex value score too low: {result.score}"
        ...
        assert np.iscomplexobj(voltage_data), "Voltage data should be complex numpy array"
        ...
        assert phase_range > 30, f"Phase range too small ({phase_range:.1f}°) - should show significant phase shift"
        
        # For RC low-pass, phase should be between 0° and -90°
        assert np.all(phase_values <= 5), f"Some phase values too positive: max={phase_values.max():.1f}°"
        assert np.all(phase_values >= -95), f"Some phase values too negative: min={phase_values.min():.1f}°"
    
    def test_rc_lowpass_behavior_validation(self, engine, rc_lowpass_circuit, behavior_validator):
        ...
        assert result.score >= 0.7, f"RC low-pass behavior score too low: {result.score}"
        ...
        assert abs(actual_cutoff - expected_cutoff) / expected_cutoff < 0.1, \
        ...
        assert abs(cutoff_gain - (-3.0)) < 1.0, f"Cutoff gain should be ~-3 dB, got {cutoff_gain:.2f} dB"
    
    def test_rc_highpass_phase_response(self, engine, rc_highpass_circuit, behavior_validator):
        ...
        complex_result = behavior_validator.validate_ac_complex_values(ac_results, node_id=2)
        ...
        if np.any(low_freq_indices):
            ...
            assert low_freq_phase > 45, f"Low frequency phase should be >45°, got {low_freq_phase:.1f}°"
        
        if np.any(high_freq_indices):
            high_freq_phase = np.mean(phase_values[high_freq_indices])
            assert abs(high_freq_phase) < 15, f"High frequency phase should be ~0°, got {high_freq_phase:.1f}°"
    
    def test_rlc_bandpass_resonance_behavior(self, engine, rlc_bandpass_circuit, behavior_validator):
        ...
        complex_result = behavior_validator.validate_ac_complex_values(ac_results, node_id=3)
        ...
        assert freq_error < 0.1, f"Resonance frequency error: {peak_frequency:.1f} Hz (expected {f_resonance:.1f} Hz)"
        
        # At resonance, phase should be approximately 0°
        phase_at_resonance = np.angle(voltage_data[peak_idx], deg=True)
        assert abs(phase_at_resonance) < 10, f"Phase at resonance should be ~0°, got {phase_at_resonance:.1f}°"
        ...
        assert peak_vs_lower > 10, f"Insufficient low-frequency attenuation: {peak_vs_lower:.1f} dB"
        assert peak_vs_upper > 10, f"Insufficient high-frequency attenuation: {peak_vs_upper:.1f} dB"
    ...
    def test_frequency_vector_generation(self, engine, rc_lowpass_circuit):
        ...
        assert frequencies_log is not None, "Should have frequency vector for decade variation"
        ...
        assert log_diff_std / log_diff_mean < 0.1, "Frequencies not properly logarithmically spaced"
        
        # Check frequency range
        assert frequencies_log[0] >= 9 and frequencies_log[0] <= 11, f"Start frequency incorrect: {frequencies_log[0]}"
        assert frequencies_log[-1] >= 9000 and frequencies_log[-1] <= 11000, f"Stop frequency incorrect: {frequencies_log[-1]}"
        ...
        assert frequencies_lin is not None, "Should have frequency vector for linear variation"
        ...
        assert lin_diff_std / lin_diff_mean < 0.1, "Frequencies not properly linearly spaced"
    
    @pytest.mark.parametrize("R_ohms,C_farads,expected_cutoff", [
        (1000, 1e-6, 159.15),    # Standard RC
        (2200, 470e-9, 153.9),   # Common values
        (10000, 100e-9, 159.15), # High R, low C
    ])
    def test_rc_filter_parameter_sweep(self, engine, behavior_validator, R_ohms, C_farads, expected_cutoff):
        ...
        assert result.passed, f"Parameter sweep failed for R={R_ohms}Ω, C={C_farads*1e6:.0f}μF: {result.issues}"
        ...
        assert cutoff_error < 0.05, f"Cutoff frequency error: {actual_cutoff:.1f} Hz (expected {expected_cutoff:.1f} Hz)"
...
class TestACAnalysisRegression:
    ...
    def test_pyspice_ac_source_configuration_bug(self):
        ...
        try:
            ...
            voltage_source = circuit.V('input', 'input', circuit.gnd, 'DC 0 AC 1')
            ...
            analysis = simulator.ac(start_frequency=100, stop_frequency=10000, number_of_points=20, variation='dec')
            ...
            for node_name, voltage_data in analysis.nodes.items():
                if 'output' in node_name:
                    ...
                    assert np.iscomplexobj(voltage_array), f"Node {node_name} should return complex values"
                    
                    # Should have meaningful phase information
                    phase_values = np.angle(voltage_array, deg=True)
                    phase_range = phase_values.max() - phase_values.min()
                    assert phase_range > 30, f"Node {node_name} phase range too small: {phase_range:.1f}°"
                    
                    # Magnitude should be reasonable
                    magnitude = np.abs(voltage_array)
                    assert np.all(magnitude > 0), f"Node {node_name} should have non-zero magnitude"
                    assert np.all(magnitude <= 1.1), f"Node {node_name} magnitude seems too high (input is 1V)"
    ...
    def test_chart_generation_with_complex_data(self, rc_lowpass_circuit):
        ...
        assert np.iscomplexobj(voltage_data), "Voltage data should be complex for proper Bode plots"
        
        # Test magnitude and phase extraction (what charts need)
        magnitude_db = 20 * np.log10(np.abs(voltage_data))
        phase_deg = np.angle(voltage_data, deg=True)
        
        assert np.all(np.isfinite(magnitude_db)), "Magnitude should be finite for charting"
        ...
        assert phase_range > 30, f"Phase range too small for meaningful Bode plot: {phase_range:.1f}°"
        ...
        assert high_freq_mag < low_freq_mag, "Low-pass filter should attenuate high frequencies"