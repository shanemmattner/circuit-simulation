...
class TestPlotStyle:
    ...
    def test_interactive_style(self):
        ...
        assert style.hover_data == True
        assert style.enable_zoom == True
...
class TestBasePlotter:
    ...
    def test_validate_data(self):
        ...
        x = np.array([1, 2, 3])
        y = np.array([4, 5, 6])
        assert plotter.validate_data(x, y) == True
        
        # Invalid - different lengths
        x = np.array([1, 2, 3])
        y = np.array([4, 5])
        assert plotter.validate_data(x, y) == False
