...
class TestCircuitSynthParser:
    ...
    def test_can_parse_circuit_synth_format(self):
        ...
        circuit_synth_content = json.dumps(
            {
                "name": "Test Circuit",
                "components": {"R1": {"symbol": "Device:R", "value": "10k"}},
                "nets": {"vcc": [{"component": "R1", "pin": "1"}]},
            }
        )
...
class TestCircuitSynthImporter:
    ...
    def test_component_mapping(self):
        ...
        assert (
            len([w for w in result.warnings if w.component_ref not in basic_components])
            >= 0
        )
    ...
    def test_error_handling(self):
        ...
        invalid_data = {"components": "not_a_dict", "nets": []}
        ...
        assert (
            ...
            or len(result.parsing_errors) > 0
        )
    ...
    def test_subcircuit_handling(self):
        ...
        circuit_data = {
            "name": "Main Circuit",
            "components": {"R1": {"symbol": "Device:R", "value": "1k"}},
            "nets": {"vcc": [{"component": "R1", "pin": "1"}]},
            "subcircuits": [
                {
                    "name": "Power Supply",
