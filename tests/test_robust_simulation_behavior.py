...
class CircuitBehaviorValidator:
    ...
    @staticmethod
    def validate_rc_lowpass_theory(
        frequencies: np.ndarray, output_voltage: np.ndarray, R: float, C: float
    ) -> Dict[str, Any]:
        ...
        results = {
            ...
            "has_phase_shift": abs(phase_sim[-1] - phase_sim[0])
            > 10,  # At least 10° shift
            "is_monotonic_mag": np.all(
                np.diff(mag_sim) <= 0.001
            ),  # Magnitude decreases (mostly)
        }
...
class TestACAnalysisBehavior:
    ...
    def test_ac_returns_complex_values(self):
        ...
        results = engine.simulate_ac(
            circuit, start_frequency=1, stop_frequency=10000, points_per_decade=10
        )
        ...
        assert (
            "complex" in validation["dtype"]
        ), f"Expected complex dtype, got {validation['dtype']}"

        # For RC filter, should have imaginary components (phase shift)
        assert validation[
            "has_imaginary_parts"
        ], "RC filter should have imaginary voltage components"
        assert (
            validation["max_imaginary"] > 1e-10
        ), f"Imaginary parts too small: {validation['max_imaginary']}"
    ...
    def test_rc_filter_physics_validation(self):
        ...
        engine = SimulationEngine()
        results = engine.simulate_ac(
            circuit, start_frequency=1, stop_frequency=100000, points_per_decade=30
        )
        ...
        assert validation[
            "has_rolloff"
        ], "RC filter must show magnitude rolloff at high frequencies"
        assert validation[
            "has_phase_shift"
        ], "RC filter must show phase shift (0° to -90°)"
        ...
        assert (
            cutoff_error < 0.1
        ), f"Cutoff magnitude error too large: {cutoff_error:.3f}"

        # Overall accuracy
        assert (
            validation["mag_error_rms"] < 0.05
        ), f"Magnitude accuracy poor: {validation['mag_error_rms']:.3f}"
        assert (
            validation["phase_error_rms"] < 10
        ), f"Phase accuracy poor: {validation['phase_error_rms']:.1f}°"
    ...
    def test_voltage_divider_ac_behavior(self):
        ...
        circuit.add_voltage_source(
            "V1", positive=1, negative="gnd", dc_value="1V"
        )  # 1V AC for testing
        circuit.add_resistor("R1", node1=1, node2=2, resistance="1k")
        ...
        ), f"Voltage divider too much variation: {magnitude_variation:.6f}"
        assert (
            abs(magnitude[0] - expected_magnitude) < 0.01
        ), f"Incorrect voltage division: {magnitude[0]:.6f} vs {expected_magnitude}"
    ...
    def test_rlc_resonance_detection(self):
        ...
        engine = SimulationEngine()
        results = engine.simulate_ac(
            circuit, start_frequency=100, stop_frequency=100000, points_per_decade=50
        )
        ...
        assert (
            freq_error < 0.2
        ), f"Resonant frequency error too large: {freq_error:.3f} ({peak_frequency:.0f} vs {f_res:.0f} Hz)"

        # Should have clear peak (Q > 1)
        low_freq_mag = magnitude[0]
        assert (
            peak_magnitude > low_freq_mag * 1.5
        ), f"Resonance peak not prominent: {peak_magnitude:.3f} vs {low_freq_mag:.3f}"
...
class TestChartGenerationRobustness:
    ...
    def test_bode_plots_contain_real_data(self):
        ...
        engine = SimulationEngine()
        results = engine.simulate_ac(
            circuit, start_frequency=1, stop_frequency=10000, points_per_decade=20
        )
        ...
        for chart_name, chart_fig in charts.items():
            assert hasattr(
                chart_fig, "data"
            ), f"Chart {chart_name} has no data attribute"
            ...
            for i, trace in enumerate(chart_fig.data):
                if hasattr(trace, "y"):
                    ...
                    assert not np.all(
                        y_data == 0
                    ), f"Chart {chart_name} trace {i} is all zeros"
                    assert not np.all(
                        y_data == y_data[0]
                    ), f"Chart {chart_name} trace {i} is flat line at {y_data[0]}"

                    # For AC magnitude plots, should see rolloff
                    if (
                        "magnitude" in trace.name.lower()
                        and "node 2" in str(trace.name).lower()
                    ):
                        data_range = y_data.max() - y_data.min()
                        assert (
                            data_range > 10
                        ), f"Insufficient magnitude rolloff in {chart_name}: {data_range:.1f}dB"
    ...
    def test_phase_plots_for_reactive_circuits(self):
        ...
        for circuit_name, circuit_func in test_circuits:
            ...
            assert np.iscomplexobj(
                output_voltage
            ), f"{circuit_name}: AC voltage should be complex"
            ...
            assert (
                phase_range > 10
            ), f"{circuit_name}: Insufficient phase shift {phase_range:.1f}° (expected >10°)"
...
class TestReportQualityAssurance:
    ...
    def test_reports_contain_interactive_charts(self):
        ...
        try:
            ...
            assert (
                len(content) > 10000
            ), f"Report too small: {len(content)} chars (expected >10k)"
            assert "Plotly.newPlot" in content, "Report missing Plotly charts"
            assert (
                content.count("Plotly.newPlot") >= 2
            ), f"Too few charts: {content.count('Plotly.newPlot')}"

            # Check for error indicators
            assert (
                "No charts available" not in content
            ), "Report shows 'No charts available' error"
            assert (
                "Chart not available" not in content
            ), "Report has chart generation errors"

            # Check for real data (not all zeros)
            assert not (
                'y":[0,0,0' in content and content.count('y":[0,0,0') > 1
            ), "Charts contain zero data"
    ...
    def test_bode_plot_data_integrity(self):
        ...
        for circuit_name, circuit_func, expected_low_freq_db in test_cases:
            ...
            engine = SimulationEngine()
            results = engine.simulate_ac(
                circuit, start_frequency=1, stop_frequency=1000, points_per_decade=10
            )
            ...
            assert not np.any(
                np.isinf(magnitude_db)
            ), f"{circuit_name}: Infinite dB values detected"
            assert not np.any(
                np.isnan(magnitude_db)
            ), f"{circuit_name}: NaN values detected"

            if expected_low_freq_db is not None:
                low_freq_error = abs(magnitude_db[0] - expected_low_freq_db)
                assert (
                    low_freq_error < 0.5
                ), f"{circuit_name}: Low freq error {low_freq_error:.2f}dB"
...
class TestLLMInformedValidation:
    ...
    def test_circuit_behavior_makes_sense(self):
        ...
        for scenario in test_scenarios:
            ...
            np.argmin(np.abs(frequencies - expected["cutoff_freq"]))
            ...
            if expected["type"] == "low_pass":
                ...
                low_freq_mag = magnitude[0]
                assert (
                    high_freq_mag < low_freq_mag * 0.5
                ), f"{scenario['name']}: No high frequency attenuation"
                ...
                high_freq_mag = magnitude[-1]
                assert (
                    low_freq_mag < high_freq_mag * 0.5
                ), f"{scenario['name']}: No low frequency attenuation"
            ...
            assert (
                phase.min() >= phase_min - 20
            ), f"{scenario['name']}: Phase too negative {phase.min():.1f}° (min expected {phase_min}°)"
            assert (
                phase.max() <= phase_max + 20
            ), f"{scenario['name']}: Phase too positive {phase.max():.1f}° (max expected {phase_max}°)"
...
class VisualRegressionTester:
    ...
    @staticmethod
    def generate_reference_bode_data(
        R: float, C: float, frequencies: np.ndarray
    ...
    def test_chart_data_vs_theory(self):
        ...
        theory_magnitude_db, theory_phase = self.generate_reference_bode_data(
            R, C, frequencies
        )
            ...
            if node != 0:
                ...
                if voltage is not None:
                    ...
                    if not np.any(voltage.imag != 0):
                        issues.append(
                            f"Node {node}: AC analysis returning real-only values (missing phase information)"
                        )
                    ...
                    if magnitude.max() - magnitude.min() < 1e-6:
                        issues.append(
                            f"Node {node}: No frequency variation (flat response)"
                        )
...
if __name__ == "__main__":
    ...
    print("   uv run pytest tests/test_robust_simulation_behavior.py -v")
