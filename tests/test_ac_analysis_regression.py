...
class TestACAnalysisRegression:
    ...
    def test_ac_results_are_complex_arrays(self, engine):
        ...
        # Run AC analysis
        ac_results = engine.simulate_ac(circuit, start_frequency=10, stop_frequency=1000, points_per_decade=5)
        ...
        assert np.iscomplexobj(voltage_data), "CRITICAL: Voltage data must be complex array"
        ...
        assert has_imaginary, "CRITICAL: Complex data must have non-zero imaginary parts"
        ...
        assert phase_range > 5, f"Phase range too small: {phase_range:.1f}° (expected >5°)"
    ...
    def test_frequency_vector_extraction(self, engine):
        ...
        ac_results = engine.simulate_ac(circuit, start_frequency=100, stop_frequency=10000, points_per_decade=10)
        ...
        assert frequencies.dtype in [np.float64, np.float32], f"Frequencies must be float, got {frequencies.dtype}"
        
        # Verify frequency range
        assert frequencies[0] >= 99 and frequencies[0] <= 101, f"Start frequency wrong: {frequencies[0]}"
        assert frequencies[-1] >= 9000 and frequencies[-1] <= 11000, f"Stop frequency wrong: {frequencies[-1]}"
        ...
        assert spacing_std / spacing_mean < 0.1, "Frequencies not logarithmically spaced"
    ...
    def test_rc_lowpass_theoretical_validation(self, engine):
        ...
        ac_results = engine.simulate_ac(circuit, start_frequency=1, stop_frequency=100000, points_per_decade=25)
        ...
        assert high_freq_phase < -80, f"High frequency phase should be < -80°, got {high_freq_phase:.1f}°"
    ...
    def test_multiple_circuits_parallel(self, engine):
        ...
        test_circuits = [
            ...
            {
                "name": "RC Low-pass",
                "components": [("V", "V1", 1, 0, "1V"), ("R", "R1", 1, 2, "1000"), ("C", "C1", 2, 0, "1e-6")],
                "expected_phase_range": (30, 90)
            },
            # RC High-pass
            {
                "name": "RC High-pass", 
                "components": [("V", "V1", 1, 0, "1V"), ("C", "C1", 1, 2, "1e-6"), ("R", "R1", 2, 0, "1000")],
                "expected_phase_range": (30, 90)
            },
            # RLC Resonant
            {
                "name": "RLC Resonant",
                "components": [("V", "V1", 1, 0, "1V"), ("R", "R1", 1, 2, "100"), ("L", "L1", 2, 3, "1e-3"), ("C", "C1", 3, 0, "1e-6")],
        ...
        for circuit_def in test_circuits:
            ...
            for comp_type, name, n1, n2, value in circuit_def["components"]:
                if comp_type == "V":
                    circuit.add_voltage_source(name, positive=n1, negative=n2, dc_value=value)
                elif comp_type == "R":
            ...
            # Run AC analysis
            ac_results = engine.simulate_ac(circuit, start_frequency=10, stop_frequency=10000, points_per_decade=15)
            ...
            assert np.iscomplexobj(voltage_data), f"{circuit_def['name']}: Not complex data"
            ...
            assert min_expected <= phase_range <= max_expected, \
    ...
    def test_ac_analysis_edge_cases(self, engine):
        ...
        ac_results = engine.simulate_ac(circuit, start_frequency=100, stop_frequency=1000, points_per_decade=2)
        voltage_data = ac_results.get_voltage(2)
        ...
        # Test single point
        ac_results = engine.simulate_ac(circuit, start_frequency=1000, stop_frequency=1000, points_per_decade=1)
        voltage_data = ac_results.get_voltage(2)
        ...
        ac_results = engine.simulate_ac(circuit, start_frequency=0.1, stop_frequency=1e6, points_per_decade=5)
        voltage_data = ac_results.get_voltage(2)
        assert np.iscomplexobj(voltage_data), "Wide range: Must preserve complex data"
        
        frequencies = ac_results.get_frequency_vector()
        assert frequencies[0] >= 0.09, f"Wide range: Start frequency wrong: {frequencies[0]}"
        assert frequencies[-1] >= 9e5, f"Wide range: Stop frequency wrong: {frequencies[-1]}"
    ...
    def test_results_compatibility_methods(self, engine):
        ...
        assert np.array_equal(voltage_old, voltage_new), "get_voltage() must match voltage()"
        assert np.array_equal(frequency_old, frequency_new), "get_frequency_vector() must match frequency"
        
        # Test non-existent nodes
        assert ac_results.get_voltage(999) is None, "Non-existent node should return None"
    ...
    def test_numpy_array_consistency(self, engine):
        ...
        ac_results = engine.simulate_ac(circuit, start_frequency=10, stop_frequency=10000, points_per_decade=10)
...
class TestACAnalysisIntegration:
    ...
    @pytest.mark.skip(reason="Chart integration test - Bode plot method needs verification")
    def test_ac_results_chart_integration(self):
        ...
        try:
            ...
            assert len(bode_fig.data) >= 2, "Bode plot should have magnitude and phase traces"
...
class TestACAnalysisPerformance:
    ...
    def test_ac_analysis_performance(self):
        ...
        ac_results = engine.simulate_ac(circuit, start_frequency=1, stop_frequency=100000, points_per_decade=20)
        elapsed_time = time.time() - start_time
        
        # Should complete within reasonable time (adjust threshold as needed)
        assert elapsed_time < 5.0, f"AC analysis too slow: {elapsed_time:.2f}s (expected <5s)"
        
        # Verify results are still correct
        voltage_data = ac_results.get_voltage(2)
        assert np.iscomplexobj(voltage_data), "Performance test: Must preserve complex data"