...
class TestComponentTypeDetector:
    ...
    def test_detect_bjt_transistor_types(self):
        ...
        assert (
            detector.detect_type("Device:Q_NPN_BCE", "Q1", "2N3904")
            == "transistor_bjt_npn"
        )
        assert (
            detector.detect_type("Device:Q_PNP_BCE", "Q2", "2N3906")
            == "transistor_bjt_pnp"
        )
        assert (
            detector.detect_type("Transistor_BJT:2N3904", "Q1", "2N3904")
            == "transistor_bjt_npn"
        )
        ...
        assert (
            detector.detect_type("unknown_symbol", "Q1", "some_value")
            == "transistor_bjt"
        )
    ...
    def test_detect_mosfet_types(self):
        ...
        assert (
            detector.detect_type("Device:Q_NMOS_GSD", "Q1", "IRF540")
            == "transistor_mosfet_n"
        )
        assert (
            detector.detect_type("Device:Q_PMOS_GSD", "Q2", "IRF9540")
            == "transistor_mosfet_p"
        )
        assert (
            detector.detect_type("Transistor_FET:IRF540N", "Q1", "")
            == "transistor_mosfet_n"
        )
    ...
    def test_detect_regulator_types(self):
        ...
        assert (
            detector.detect_type("Regulator_Linear:L7805", "U1", "")
            == "regulator_linear"
        )
        assert detector.detect_type("Device:78L05", "U2", "") == "regulator_linear"
        assert (
            detector.detect_type("Regulator_Switching:LM2596", "U3", "")
            == "regulator_switching"
        )
...
class TestFuzzySymbolMatch:
    ...
    def test_fuzzy_transistor_match(self):
        ...
        self.mock_loader.get_available_models.return_value = [
            "2N3904",
            "2N3906",
            "BC547",
        ]
...
class TestCircuitIntegration:
    ...
    def test_circuit_bjt_addition(self):
        ...
        circuit.add_bjt_transistor(
            "Q1", collector="2", base="1", emitter="0", model="2N3904"
        )
    ...
    def test_circuit_opamp_addition(self):
        ...
        circuit.add_opamp(
            "U1", vplus="1", vminus="2", vout="3", vcc="4", vee="0", model="LM358"
        )
