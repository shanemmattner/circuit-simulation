...
class TestTransferFunctionExtraction:
    ...
    def test_rc_lowpass_transfer_function(self):
        ...
        poles = tf.poles
        assert all(np.real(pole) < 0 for pole in poles), "All poles should be in left half-plane"
        
        # Verify bandwidth is reasonable (around 159 Hz for 1kΩ, 1μF)
        bandwidth_hz = tf.bandwidth / (2 * np.pi)
        assert 100 < bandwidth_hz < 300, f"Expected bandwidth ~159Hz, got {bandwidth_hz:.1f}Hz"
    ...
    def test_rlc_bandpass_transfer_function(self):
        ...
        if tf.order >= 2:
            assert len(tf.poles) >= 2, "Second-order system should have at least 2 poles"
    ...
    def test_transfer_function_mathematical_representation(self):
        ...
        assert hasattr(tf, 'denominator_coeffs'), "TF should have denominator coefficients"
        
        # Test coefficient arrays
        num_coeffs = tf.numerator_coeffs
        den_coeffs = tf.denominator_coeffs
        
        assert isinstance(num_coeffs, np.ndarray), "Numerator coeffs should be numpy array"
        assert isinstance(den_coeffs, np.ndarray), "Denominator coeffs should be numpy array"
        ...
        assert np.all(np.isfinite(num_coeffs)), "Numerator coefficients should be finite"
        assert np.all(np.isfinite(den_coeffs)), "Denominator coefficients should be finite"
    ...
    def test_transfer_function_edge_cases(self):
        ...
        results.add_voltage("out", np.array([1+0j, 0.7+0j, 0.1+0j]))  # Simple decay
    ...
    def test_transfer_function_stability_analysis(self):
        ...
        if len(poles) > 0:
            ...
            assert np.all(pole_real_parts < 1e-6), f"Poles should be in LHP, got real parts: {pole_real_parts}"
...
class TestTransferFunctionMetrics:
    ...
    def test_metrics_calculation(self):
        ...
        assert isinstance(metrics['tf_dc_gain'], (int, float)), "DC gain should be numeric"
    ...
    def test_latex_formatting(self):
        ...
        assert '(' in readable_eq and ')' in readable_eq, "Readable should contain parentheses"
    ...
    def test_filter_characteristics(self):
        ...
        if 'filter_type' in metrics:
            ...
            assert filter_type in ['low-pass', 'high-pass', 'band-pass', 'unknown'], f"Unexpected filter type: {filter_type}"
...
class TestTransferFunctionFromFrequencyResponse:
    ...
    def test_from_frequency_response_first_order(self):
        ...
        for idx in test_indices:
            ...
            actual = extracted_response[idx] if len(extracted_response) > idx else extracted_response[0]
            
            # Allow for some fitting error (within 20% magnitude error)
            mag_error = abs(abs(actual) - abs(expected)) / abs(expected)
            assert mag_error < 0.5, f"Magnitude error too large at index {idx}: {mag_error:.2%}"
    ...
    def test_from_frequency_response_edge_cases(self):
        ...
        assert abs(tf.dc_gain - 1.0) < 0.1, "DC gain should be close to 1 for constant response"
