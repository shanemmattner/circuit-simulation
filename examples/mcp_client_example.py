    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            validation = await session.call_tool(
                "circuit.validate", arguments={"circuit_id": circuit_id}
            )
            ...
            dc_result = await session.call_tool(
                "simulation.run_dc", arguments={"circuit_id": circuit_id}
            )
            ...
            for comp in rc_components:
                await session.call_tool(
                    "circuit.add_component", arguments={"circuit_id": rc_id, **comp}
                )
            ...
            if transient_data["status"] == "success":
                ...
                for node, stats in transient_data["results"]["node_voltages"].items():
                    print(
                        f"  Node {node}: min={stats['min']:.3f}V, max={stats['max']:.3f}V, final={stats['final']:.3f}V"
                    )
            ...
            results = await session.call_tool(
                ...
                arguments={"circuit_id": rc_id, "simulation_type": "transient"},
            )
            ...
            # 9. List all circuits
            print("\n=== All Circuits ===")
            list_result = await session.call_tool("circuit.list", arguments={})
