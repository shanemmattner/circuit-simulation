    ...
    if circuit.damping_type == "underdamped":
        ...
        for i in range(1, len(time)):
            ...
            if circuit.topology == "series":
                ...
                voltage[i] = input_signal[i] * (1 - np.exp(-zeta*omega_n*t) * 
                            (np.cos(omega_d*t) + (zeta*omega_n/omega_d)*np.sin(omega_d*t)))
                # Current through circuit
                current[i] = (input_signal[i] / circuit.r) * np.exp(-zeta*omega_n*t) * np.sin(omega_d*t)
            else:  # parallel
                # Different response for parallel
                voltage[i] = input_signal[i] * np.exp(-zeta*omega_n*t) * np.cos(omega_d*t)
                current[i] = (input_signal[i] / circuit.r) * (1 - np.exp(-zeta*omega_n*t) * np.cos(omega_d*t))
        ...
        for i in range(1, len(time)):
            ...
            if circuit.topology == "series":
                ...
                current[i] = (input_signal[i] / circuit.r) * (1 - np.exp(-omega_n*t) * (1 + omega_n*t))
