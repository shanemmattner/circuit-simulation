...
def _create_bode_plot(
    ...
    if "magnitude_db" not in spectrum:
        ...
        if "magnitude" in spectrum:
            spectrum["magnitude_db"] = [20*np.log10(m) if m > 0 else -100 for m in spectrum["magnitude"]]
    ...
    fig.add_shape(
        type="line",
        x0=f_lower, y0=f0_mag-3,
        x1=f_upper, y1=f0_mag-3,
...
def generate_3d_response(
    ...
    if vary_param == "q":
        ...
        param_label = {"r": "Resistance (Ω)", "l": "Inductance (H)", "c": "Capacitance (F)"}[vary_param]
