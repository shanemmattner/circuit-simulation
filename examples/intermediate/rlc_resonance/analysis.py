...
def analyze_resonance(circuit: RLCResonanceCircuit) -> Dict[str, Any]:
    ...
    return {
        ...
        "rise_time": 1 / (circuit.damping_ratio * circuit.angular_frequency) if circuit.damping_ratio > 0 else float('inf'),
        "settling_time": 4 / (circuit.damping_ratio * circuit.angular_frequency) if circuit.damping_ratio > 0 else float('inf')
    }
...
def analyze_stability(circuit: RLCResonanceCircuit) -> Dict[str, Any]:
    ...
    return {
        ...
        "oscillation_frequency": circuit.damped_frequency if circuit.damping_type == "underdamped" else 0
    }
