...
def main():
    ...
    try:
        ...
        for name, info in power_analysis.component_power.items():
            print(f"  {name}: {info.power:.3f}W @ {info.voltage:.3f}V, {info.current:.3f}A ({info.method})")
        ...
        print(f"Rating Validation: {'✅ PASS' if rating_analysis.is_valid else '❌ FAIL'}")
        
        for name, info in rating_analysis.component_power.items():
            rating = info.rating
            if rating:
                utilization = (info.power / rating) * 100
                status = "✅" if utilization < 80 else "⚠️" if utilization < 100 else "❌"
                print(f"  {name}: {info.power:.3f}W / {rating}W ({utilization:.1f}%) {status}")
        ...
        max_component_power = max([info.power for info in power_analysis.component_power.values()])
        if max_component_power > 0.5:
            print(f"  - Component with highest power ({max_component_power:.3f}W) may need heat sink")
