...
def demo_nyquist_plots():
    ...
    for style_name, style in styles.items():
        ...
        for sys_name, sys_data in systems.items():
            ...
            print(
                f"  - {sys_name}: Stable = {result.metadata['stability_analysis']['is_stable']}"
            )
            plt.savefig(
                f"nyquist_{sys_name.lower().replace(' ', '_')}_{style_name}.png",
                dpi=style.dpi,
                bbox_inches="tight",
            )
            plt.close()
...
def demo_smith_charts():
    ...
    z_in = (
        z0_line
        * (1 + gamma_l * np.exp(-2j * beta_l * 0.1))
        / (1 - gamma_l * np.exp(-2j * beta_l * 0.1))
    )
    ...
    for scenario_name, scenario_data in scenarios.items():
        ...
        plt.savefig(
            f"smith_{scenario_name.lower().replace(' ', '_')}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def demo_nichols_charts():
    ...
    for sys_name, sys_data in systems.items():
        ...
        plt.savefig(
            f"nichols_{sys_name.lower().replace(' ', '_')}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def run_performance_benchmark():
    ...
    plotter.plot(transfer_function, frequencies, title="Benchmark Test")
