...
def _create_dc_analysis_plot(
    ...
    fig.add_trace(
        go.Bar(x=components, y=voltages, name="Voltage"),
        row=1, col=1
    )
    ...
    fig.update_layout(
        title="Voltage Divider DC Analysis",
        showlegend=False,
        height=800
    )
...
def _create_sweep_plot(results: Dict[str, Any]) -> go.Figure:
    ...
    if results["sweep_param"] == "vin":
        ...
        ideal_ratio = results["output_voltages"][1] / results["sweep_values"][1] if results["sweep_values"][1] > 0 else 0
        fig.add_trace(go.Scatter(
            x=results["sweep_values"],
