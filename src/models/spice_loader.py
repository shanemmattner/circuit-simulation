...
class SpiceModelLoader:
            ...
            for file_path in self.models_path.rglob(ext):
                ...
                # Also check file content for model definition
                    ...
                    try:
                        ...
                        if (
                            f".model {search_pattern}" in content
                            or f".subckt {search_pattern}" in content
                        ):
                            results.append(file_path)
                    except:
                        pass
    ...
    def _search_and_load(
        self, model_name: str, search_paths: List[Path]
    ) -> Optional[str]:
            ...
            for name_var in name_variations:
                for ext in [".lib", ".mod", ".LIB", ".MOD"]:
                    ...
                    if file_path.exists():
                        ...
                        if (
                            content.count(".model") == 1
                            or content.count(".subckt") == 1
                        ):
                            return content
                    ...
                    for file_path in base_path.rglob(ext):
                        ...
                        if model_name.lower() in file_path.stem.lower():
                            ...
                            if file_path.stem.lower() == model_name.lower():
                                if (
                                    content.count(".model") <= 2
                                    and content.count(".subckt") <= 2
                                ):
                                    return content
        ...
        for line in lines:
            ...
            if not in_model:
                for name in model_names:
                    if (
                        f".model {name.lower()}" in line_lower
                        or f".subckt {name.lower()}" in line_lower
                    ):
                        in_model = True
            ...
            # If we're in a model, collect lines
                ...
                # Check for start of next model/subckt (end of current model)
                    ...
                    ".model"
                ) or line_lower.strip().startswith(".subckt"):
                    break
    ...
    def _format_resistor(self, value: float) -> str:
        ...
        if value >= 1e6:
            ...
            return "R value=1k" if value == 1000 else f"R value={value/1e3:.1f}k"
        else:
    ...
    def _format_capacitor(self, value: float) -> str:
        ...
        if value >= 1e-6:
            return "C value=1u" if value == 1e-6 else f"C value={value*1e6:.1f}u"
        elif value >= 1e-9:
    ...
    def _format_inductor(self, value: float) -> str:
        ...
        if value >= 1e-3:
            return "L value=1m" if value == 1e-3 else f"L value={value*1e3:.1f}m"
        elif value >= 1e-6:
