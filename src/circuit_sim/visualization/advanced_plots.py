...
class NyquistPlotter(BasePlotter):
    ...
    def plot(
        ...
        if include_negative_freq:
            ax.plot(real, -imag, 'b--', linewidth=self.style.line_width * 0.7, 
                   alpha=0.7, label='H(-jω)')
        ...
        ax.plot(real[0], imag[0], 'go', markersize=self.style.marker_size, label='Low freq')
        ax.plot(real[-1], imag[-1], 'ro', markersize=self.style.marker_size, label='High freq')
        
        # Mark critical point (-1, 0)
        if show_critical_point:
            ax.plot(-1, 0, 'rx', markersize=self.style.marker_size * 1.5, 
                   markeredgewidth=3, label='Critical point')
            ax.plot(-1, 0, 'ro', markersize=self.style.marker_size * 2, 
        ...
        if m_db == 0:  # 0 dB circle
            ...
            if m_linear < 1:
                ...
                for phi in phase_rad:
                    ...
                    mag = m_linear / np.sqrt((1 - m_linear**2) * np.cos(phi)**2 + (m_linear * np.sin(phi))**2)
...
def nichols_to_bode(phase_nichols: np.ndarray, magnitude_nichols: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
def calculate_closed_loop_response(ol_magnitude_db: np.ndarray, ol_phase_deg: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def plot(self,
        ...
        if show_grid:
            ...
            for circle in m_circle_data:
                ax.plot(circle["phase_points"], circle["magnitude_points"],
        ...
        if mark_frequencies:
            for freq in mark_frequencies:
                idx = np.argmin(np.abs(frequencies - freq))
                ax.plot(phase_deg[idx], magnitude_db[idx], 
        ...
        # Mark start and end points
        ax.plot(phase_deg[0], magnitude_db[0], 
               'go', markersize=self.style.marker_size, label='Low freq')
        ax.plot(phase_deg[-1], magnitude_db[-1], 
               'ro', markersize=self.style.marker_size, label='High freq')
        ...
        if show_margins:
            ...
            margins = calculate_stability_margin(magnitude_linear, phase_rad, frequencies)
