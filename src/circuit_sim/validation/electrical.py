...
class ShortCircuitDetector(ValidationRule):
            ...
            if component.get("type") == "voltage_source":
                ...
                if "dc_value" in component:
                    try:
                        ...
                        voltage_str = (
                            str(component["dc_value"]).upper().replace("V", "")
                        )
                ...
                sources.append(
                    {
                        "name": component["name"],
                        "positive": component.get(
                            "positive", component.get("node1", 0)
                        ),
                        "negative": component.get(
                            "negative", component.get("node2", 0)
                        ),
                        "voltage": voltage,
                        "component": component,
                    }
    ...
    def _get_component_resistance(self, component) -> float:
        ...
        if "resistance" in component:
            try:
                ...
                resistance_str = resistance_str.replace("K", "000").replace(
                    "M", "000000"
                )
    ...
    def _find_path_resistance(
        self, graph: Dict[int, List[Tuple[int, float]]], source1: Dict, source2: Dict
    ) -> Optional[float]:
        """
        ...
        if (
            source1["positive"] == source2["positive"]
            and source1["negative"] == source2["negative"]
        ) or (
            source1["positive"] == source2["negative"]
            and source1["negative"] == source2["positive"]
        ):
        ...
        if source1["negative"] == 0 and source2["negative"] == 0:
            ...
            resistance = self._dijkstra_min_resistance(
                graph, source1["positive"], source2["positive"]
            )
            return resistance
    ...
    def _dijkstra_min_resistance(
        self, graph: Dict[int, List[Tuple[int, float]]], start: int, end: int
    ) -> Optional[float]:
        """
