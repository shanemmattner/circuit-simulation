...
class CircuitAPIClient:
    ...
    def get_circuit_options(self) -> List[Dict[str, str]]:
        ...
        return [
            {"label": circuit["name"], "value": circuit["id"]} for circuit in circuits
        ]
    ...
    def start_simulation(
        self, circuit_id: str, simulation_type: str, parameters: dict = None
        ...
        payload = {"type": simulation_type, "parameters": parameters}

        log_api_request(logger, "POST", url)
        logger.info(
            f"Starting {simulation_type} simulation with parameters: {parameters}"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Simulation failed: {response.status_code} - {response.text}"
                )
                return None
        except requests.exceptions.RequestException as e:
