...
@app.callback(
    Output("circuit-selector", "options"), Input("page-load-trigger", "children")
)
def load_circuits(trigger):
    ...
    try:
        ...
        log_callback_execution(
            gui_logger, "load_circuits", {"trigger": trigger}, success=True
        )
        ...
        gui_logger.info(
            f"Successfully loaded circuits: {[opt['label'] for opt in options]}"
        )
        return options
...
def update_tab_content(selected_circuit, active_tab):
    ...
    gui_logger.info(
        f"Updating tab content - Circuit: {selected_circuit}, Tab: {active_tab}"
    )
    ...
    try:
        ...
        circuit_name = (
            circuit_details.get("name", "Unknown") if circuit_details else "Unknown"
        )

        if active_tab == "dc-analysis":
            return create_dc_analysis_content(
                selected_circuit, circuit_name, circuit_details
            )
            ...
            return html.Div(
                [
                    ...
                    html.H5(
                        f"Analysis: {active_tab.replace('-', ' ').title()}",
...
def run_dc_analysis(n_clicks, circuit_id):
    ...
    try:
        ...
        if status == "failed":
            return html.Div(
                [
                    html.P("❌ Simulation Failed", className="text-danger fw-bold"),
...
def create_dc_analysis_content(
    circuit_id: str, circuit_name: str, circuit_details: dict
) -> html.Div:
                            ...
                            dbc.Card(
                                [
                                    ...
                                    dbc.CardBody(
                                        [
                                            html.P(
                                                "Click 'Run DC Analysis' to see results.",
                                ...
                                [
                                    ...
                                    dbc.CardBody(
                                        [
                                            ...
                                            html.P(
                                                "🎯 Interactive cursors for measurements"
                                            ),
...
def create_transient_analysis_content(circuit_id: str, circuit_name: str) -> html.Div:
    ...
    return html.Div(
        [
            html.H4(
                f"⚡ Transient Analysis: {circuit_name}", className="text-primary mb-3"
            ),
            dbc.Row(
                [
                    dbc.Col(
                                ...
                                [
                                    ...
                                    dbc.CardBody(
                                        [
                                            ...
                                            html.P(
                                                "📏 Rise time, settling time measurements"
                                            ),
                                ...
                                [
                                    ...
                                    dbc.CardBody(
                                        [
                                            ...
                                            html.Br(),
                                            dbc.Badge(
                                                "📡 WebSocket Connected", color="info"
                                            ),
                                        ]
                                    ),
                                ]
