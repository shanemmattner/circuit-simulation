...
class CircuitSynthImporter:
        ...
        for ref, comp_data in components_data.items():
            try:
                ...
                self.failed_components.append(
                    {"reference": ref, "data": comp_data, "error": str(e)}
                )
    ...
    def _add_component(
        ...
        if value and self.value_extractor:
            try:
                ...
                extract_result = self.value_extractor.extract_value(
                    component_section, ref, symbol
                )
        ...
        if symbol in self.COMPONENT_MAP:
            ...
            self._add_mapped_component(
                circuit,
                ref,
                component_type,
                parsed_value or value,
                connected_nets,
                comp_data,
            )
        elif any(
            pattern in symbol for pattern in ["Device:R", "resistor"]
        ) or ref.startswith("R"):
            self._add_resistor(
                circuit, ref, parsed_value or value or "1k", connected_nets
            )
        elif any(
            pattern in symbol for pattern in ["Device:C", "capacitor"]
        ) or ref.startswith("C"):
            self._add_capacitor(
                circuit, ref, parsed_value or value or "1uF", connected_nets
            )
        elif any(
            pattern in symbol for pattern in ["Device:L", "inductor"]
        ) or ref.startswith("L"):
            self._add_inductor(
                circuit, ref, parsed_value or value or "1mH", connected_nets
            )
        elif any(
            pattern in symbol for pattern in ["power:", "voltage"]
        ) or ref.startswith("V"):
            self._add_voltage_source(
                circuit, ref, parsed_value or value or "5V", connected_nets
            )
        else:
            # Use model mapper for advanced components if available
            if self.model_mapper:
                try:
                    self.model_mapper.map_component(
                        circuit, ref, symbol, value, connected_nets
                    )
                except Exception as e:
    ...
    def _find_component_nets(
        self, ref: str, net_map: Dict[str, Set[Tuple[str, str]]]
    ) -> List[str]:
    ...
    def _add_voltage_source(
        self, circuit: Circuit, ref: str, value: str, nets: List[str]
    ):
    ...
    def _add_mapped_component(
        ...
        if component_type in ["resistor", "capacitor", "inductor", "voltage_source"]:
            ...
            self._add_behavioral_model(
                circuit, ref, comp_data.get("symbol", "unknown"), nets
            )

    def _add_behavioral_model(
        self, circuit: Circuit, ref: str, symbol: str, nets: List[str]
    ):
