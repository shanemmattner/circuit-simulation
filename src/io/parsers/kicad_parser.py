...
class KiCadParser:
        ...
        for ref, comp_data in components.items():
            ...
            if symbol == "R" or ref.startswith("R"):
                circuit.add_resistor(
                    ref, comp_nodes.get("1", 1), comp_nodes.get("2", 0), value
                )
    ...
    def parse_content_with_result(self, content: str) -> ImportResult:
        ...
        try:
            ...
            if nets:
                ...
                result.add_parsing_error(
                    "No nets section found - components will have default connections"
                )
    ...
    def _parse_components_robust(
        self, content: str, result: ImportResult, format_info: FormatInfo
    ):
        ...
        try:
            ...
            for ref, comp_data in components.items():
                try:
                    self._process_single_component_robust(
                        ref, comp_data, content, result
                    )
                except Exception as e:
                    failure = create_component_failure(
                        ref,
    ...
    def _process_single_component_robust(
        self, ref: str, comp_data: dict, full_content: str, result: ImportResult
    ):
        ...
        if extracted_value and extracted_value.strip() and extracted_value != '""':
            ...
            value_result = self.value_extractor.extract_value(full_content, ref, symbol)
        ...
        try:
            success = self._create_circuit_component(
                result.circuit, ref, symbol, value_result.value
            )
            ...
            failure = create_component_failure(
                ref, f"Error creating component: {str(e)}", FailureLevel.ERROR
            )
            result.add_failure(failure)

    def _create_circuit_component(
        self, circuit: Circuit, ref: str, symbol: str, value: str
    ) -> bool:
    ...
    def _create_advanced_component(
        self, circuit: Circuit, ref: str, symbol: str, value: str
    ) -> bool:
        ...
        try:
            ...
            if component_model.type.startswith("transistor_bjt"):
                ...
                component_model.type.startswith("transistor_mosfet")
                or component_model.type == "mosfet"
            ):
                circuit.add_mosfet(
                    ref,
    ...
    def _apply_connectivity_robust(
        self, circuit: Circuit, nets: dict, result: ImportResult
    ):
    ...
    def _parse_net_line(self, line: str, circuit: Circuit):
        ...
        for ref, comp_data in self.components.items():
            ...
            if ref.startswith("R") and ref not in [
                c.get("name", "") for c in circuit.components
            ]:
                # Add resistor if not already added
                circuit.add_resistor(ref, "1", "0", value)  # Simplified nodes
            elif ref.startswith("C") and ref not in [
                c.get("name", "") for c in circuit.components
            ]:
                circuit.add_capacitor(ref, "1", "0", value)
            elif ref.startswith("L") and ref not in [
                c.get("name", "") for c in circuit.components
            ]:
