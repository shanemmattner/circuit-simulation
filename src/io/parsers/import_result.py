...
class ComponentFailure:
    ...
    def __str__(self):
        ...
        result = (
            f"{self.level.value.upper()}: {self.component_ref} - {self.error_message}"
        )
...
class ImportResult:
    ...
    def is_successful(self) -> bool:
        ...
        return (
            len([f for f in self.failed_components if f.level == FailureLevel.CRITICAL])
            == 0
        )
    ...
    def summary(self) -> str:
        ...
        error_count = len(
            [f for f in self.failed_components if f.level == FailureLevel.ERROR]
        )
        critical_count = len(
            [f for f in self.failed_components if f.level == FailureLevel.CRITICAL]
        )
        ...
        if critical_count > 0:
            ...
            lines.append("⚠️  Import completed with issues")
        else:
        ...
        if self.has_warnings:
            ...
            all_issues = [(f, f.level.value) for f in self.failed_components] + [
                (w, "warning") for w in self.warnings
            ]
            all_issues.sort(
                key=lambda x: {"critical": 0, "error": 1, "warning": 2}[x[1]]
            )
            ...
            lines.append("")
            lines.append(
                "💡 Use result.failed_components and result.warnings for full details"
            )
...
def _get_default_suggestion(error_message: str, component_ref: str) -> str:
    ...
    if "value" in error_lower and "empty" in error_lower:
        return (
            f"Add a value for {component_ref} in KiCad, or set it manually after import"
        )
