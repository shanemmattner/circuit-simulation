...
class ValueExtractor:
    ...
    def extract_value(
        self, component_section: str, ref: str, part_type: str = ""
    ...
    def _extract_inline_value(
        self, content: str, ref: str, part_type: str
    ) -> ValueExtractionResult:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if value and value != "" and value != '""' and value != "''":
                    return ValueExtractionResult(
                        value=value, confidence=0.9, method="inline_value"
                    )
    ...
    def _extract_multiline_value(
        ...
        next_comp = re.search(
            r"\(comp\s+\(ref\s+(?!" + re.escape(ref) + r")", remaining_text
        )
        ...
        if value_match:
            ...
            if value and value != "":
                return ValueExtractionResult(
                    value=value, confidence=0.8, method="multiline_value"
                )

        return ValueExtractionResult(None, 0.0, "multiline_value")

    def _extract_from_component_block(
        self, content: str, ref: str, part_type: str
    ) -> ValueExtractionResult:
        ...
        pattern = (
            rf"\(comp\s+\(ref\s+{re.escape(ref)}\).*?\)\s*(?=\(comp|\(libparts|\)$)"
        )
        ...
        for pattern in value_patterns:
            ...
            if match:
                ...
                if value and value != "" and value != '""' and value != "''":
                    return ValueExtractionResult(
                        value=value, confidence=0.7, method="component_block"
                    )

        return ValueExtractionResult(None, 0.0, "component_block")

    def _extract_from_part_name(
        self, content: str, ref: str, part_type: str
    ) -> ValueExtractionResult:
        ...
        value_patterns = {
            ...
            r"(\d+[uU][fF])\b": lambda m: m.group(1)
            .replace("U", "u")
            .replace("F", "F"),  # 100UF -> 100uF
