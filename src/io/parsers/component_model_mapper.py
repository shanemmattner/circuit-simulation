    ...
    def __init__(self):
        ...
        self.symbol_patterns = {
            ...
            "diode_signal": [
                r".*:D$",
                r".*:1N41[0-9]+.*",
                r".*Signal.*",  # 1N41xx series are signal diodes
            ],
            "diode_power": [
                r".*:1N4[0-9]+.*",
                r".*Power.*Diode.*",
            ...
            "diode_zener": [r".*D_Zener.*", r".*Zener.*", r".*:1N[0-9]+Z.*"],
            "diode_led": [r".*LED.*", r".*Light.*Emitting.*"],
            # Operational Amplifiers
            "opamp": [
                r".*Amplifier_Operational.*",
                r".*OpAmp.*",
                r".*:LM[0-9]+.*",
                r".*:TL[0-9]+.*",
                r".*:AD[0-9]+.*",
...
class ExactSymbolMatch:
    ...
    def find_model(
        self, kicad_symbol: str, reference: str, value: str
    ...
    def _try_value_match(self, model_name: str, comp_type: str) -> Optional[str]:
        ...
        try:
            ...
            if comp_type.startswith("transistor_bjt"):
                ...
                return self.model_loader.load_transistor(
                    model_name
                )  # Same loader for now
...
class FuzzySymbolMatch:
    ...
    def _find_similar_model(
        self, target: str, comp_type: str
    ...
    def _get_available_models_for_type(self, comp_type: str) -> List[str]:
        ...
        try:
            if comp_type.startswith("transistor"):
                ...
                return (
                    self.model_loader.get_available_models("Operational Amplifier")
                    or []
                )
            else:
...
class DefaultBehavioral:
    ...
    def find_model(
        ...
        model_generator = self.behavioral_models.get(
            comp_type, self.behavioral_models["unknown"]
        )
...
class ComponentModelMapper:
    ...
    def get_mapping_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_confidence_rate": len([c for c in confidences if c > 0.7])
            / len(confidences),
        }
